...
depresion$Comunidades.y.Ciudades.Autónomas[depresion$Comunidades.y.Ciudades.Autónomas == ""] <- "Total"
datatable(depresion)
```
...
comparacion <- merge(depresion, alcohol, by = "Comunidades.y.Ciudades.Autónomas")
names(comparacion)
tabla_final<-comparacion[ ,c("Comunidades.y.Ciudades.Autónomas", "Total.x", "Total.y")]
colnames(tabla_final)<-c("Comunidades Autónomas", "Prevalencia Depresión", "Consumo Alcohol")
...
```
```{r}
html_products <- read_html(html)

# Encuentra la tabla específica por el encabezado
tabla_andalucia <- html_nodes(html_products, "table:contains('HORAS DE SOL EN ANDALUCÍA')")
