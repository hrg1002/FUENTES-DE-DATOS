...
a <- select(salarios, Nombre, Data)%>%
  dplyr::filter(grepl("decil", Nombre) & !grepl("Jornada", Nombre))%>%
  mutate(Comunidades.y.Ciudades.Autónomas = substr(Nombre, 7, nchar(Nombre)-41))%>%
  select(Comunidades.y.Ciudades.Autónomas, Data)
